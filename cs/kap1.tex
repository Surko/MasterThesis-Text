...
V oddieli popíšeme, čo sú to dáta. Zadefinujeme tie najzákladnejšie pojmy, ktoré budeme v práci používať. Informácie sme čerpali z voľne dostupnej knihy  \cite{kap1-DataMiningAndAnalysis} a online zdrojov \cite{wiki-Data,online-Data}.

V prvej časti \ref{kap1:2.1:2.1.1:DataRepresentation} popíšeme, čo sú to dáta a ako ich môžme reprezentovať. Popíšeme základné pojmy, ako je \emph{atribút} a \emph{príznakový vektor}. V časti \ref{kap1:2.1:2.1.2:DataAttributes} rozdelíme atribúty na dva druhy - kategoriálne a numerické. Aj napriek jednoduchosti týchto pojmov ich treba zaviesť, pretože ich budeme využívať v neskorších kapitolách.
...
Zozbierané dáta sú väčšinou uschované v relačných, tabuľkových databázach. Tento typ dát nazývame štrukturované. V skutočnosti sa však vo väčšine prípadov stretávame presne s ich opakom, a to s neštrukturovanými dátami.
Podľa \cite{kap1-DataMiningAndAnalysis} si dáta môžeme predstaviť ako maticu $n \times d$, kde $n$ predstavuje počet riadkov a $d$ počet stĺpcov tejto matice. Riadky sú v tomto prípade objekty a stĺpce sú tvorené hodnotami danej vlastnosti. Matica je daná takto
...
Jeden riadok matice nazývame príznakový vektor, entita, objekt, transakcia alebo inštancia.
\begin{center}
$\mathbf{x}_{i} = (x_{i1},x_{i2},\ldots,x_{id})$
\end{center}

$X_{j}$ predstavuje $j$-tý stĺpec, ktorý obsahuje $n$ hodnôt.
Stĺpec matice nazývame príznak, vlastnosť, atribút.
\begin{center}
...
\item \textbf{Usporiadané}, keď sa hodnoty v doméne dajú porovnávať. Takéto hodnoty môžeme porovnávať a nie len na zhodu, ale aj či je hodnota väčšia, menšia. Ako príklad môžeme uviesť vzdelanie (základné, stredné, vysokoškolské).
\end{itemize}
...
Nakoniec v oddieli \ref{kap1:2.2:2.2.5:Tools} zhrnieme a uvedieme pár základných nástrojov, ktoré môžeme použiť pri práci v obore dobývania znalostí. 
...
Dobývanie znalostí je technika z KDD, ktorá je priamo zodpovedná za vytváranie poznatkov o existujúcich dátach, tak isto ako za predikciu určitých vlastnosti dát. Je to obor, ktorý kombinuje viaceré poznatky zo štatistiky, umelej inteligencie, strojového učenia a zo znalostí databázových strojov (Obrázok \ref{fig:dataMineComb}).
...
Verifikačné metódy spoliehajú na určitú predom známu hypotézu, ktorá býva vyhodnotená expertom. Tieto metódy nie sú až tak spájané s dobývaním znalostí, ako to je v prípade objaviteľských metód a to kvôli tomu, že techniky dobývania znalostí by mali vyberať hypotézu z množiny vhodných hypotéz (identifikácia modelu), než aby sa spoliehali na nejakú dopredu známu (odhadnutie modelu).
...
Učenie bez učiteľa je presným opakom učenia s učiteľom. Tento typ vytvára model bez trénovacej množiny. Podľa taxonómie sem môžeme zase zaradiť popisné metódy (nie vizualizáciu) z objaviteľských techník. 

\subsection{Nástroje pre DM}\label{kap1:2.2:2.2.5:Tools}
Existuje veľké množstvo nástrojov, ktoré boli vytvorené tak, aby zvládali úlohy z dobývania znalostí. Medzi nimi existuje plno profesionálnych nástrojov určených priamo pre takéto úlohy (Weka, RapidMiner). Iné druhy sú vo forme knižníc do stávajúcich jazykov (Python, Matlab, ...). Tieto nástroje sú implementované tak, aby dokázali použiť techniky umelej inteligencie, strojového učenia, štatistiky a iné. Veľa z nich je ale platených, niektoré zase komplikované na inštaláciu, konfiguráciu alebo použitie. Pre učenie, ľahšiu využiteľnosť a hlavne pre účel našej práce sú preto nevhodné. 
...
Pre orientované hrany existuje ešte druhý typ súvislosti nazývaný \emph{silná}. Avšak táto súvislosť je pre naše účely, a hlavne definíciu rozhodovacích stromov nepodstatná.
...
Na Obrázku \ref{fig:decisionPlaneDataa} sú zobrazené štyri množiny dát, ktoré spolu tvoria celú množinu $D$ ($=D_{1} \cup D_{2} \cup D_{3} \cup D_{4}$). Farba jednotlivých útvarov určuje ich klasifikáciu.
...
Rozhodovací strom, ako už vieme z predchádzajúcej kapitoly, je hierarchická štruktúra, ktorá pomocou rozhodnutí zobrazí zadaný vektor na určitú výstupnú hodnotu. V obore dobývania znalostí je rozhodovací strom prediktívny model, ktorý môže reprezentovať ako klasifikačný, tak aj regresný model. Keď je strom použitý na klasifikačné úlohy, tak ho nazývame klasifikačný.
...
Konkrétnejšie budú niektoré z indukčných algoritmov popísané v oddieli \ref{kap1:2.7:DTTechniques}. 

\subsection{Regresný strom}\label{kap1:2.4:2.4.1:DTRegression}
Regresný strom je variantou rozhodovacieho stromu navrhnutý na aproximáciu reálnych funkcií. Strom je veľmi podobný tomu klasifikačnému. Jediným rozdielom je výstupný atribút, ktorého doména je v tomto prípade nekonečná množina ($\mathbb{R},\mathbb{N},\ldots$).
...
Tento druh stromov teda používame vtedy, keď chceme predikovať vlastnosť objektu, ktorá je z nekonečnej množiny. Použitie v praxi je úplne rovnaké, ako v prípade klasifikačných stromov. Jednou z výhod je, že regresné stromy majú širšie možnosti predikcie. Týka sa to hlavne ďalšieho použitia iných druhov modelov v koncových listoch. Takto vytvorené stromy bývajú o niečo náročnejšie na konštrukciu kvôli ich komplikovanejšej štruktúre. Výsledky získané z regresných stromov sú vďaka ich zložitejšej štruktúre zvyčajne lepšie než pri použití iných, jednoduchších metód.

Na Obrázku \ref{fig:TypesRegression} je vykreslený typický regresný strom. V tomto prípade predikuje výšku priradenej pôžičky žiadateľom podľa ich veku a sociálneho zaradenia (príklad nevychádza z reálnych údajov). Označme žiadateľa/entitu o pôžičku ako $E$. Jednotlivé pravidlá pre tento strom by vyzerali takto:
\begin{align}
...
Gini index je ďalšie z kritérií založené na neusporiadanosti. Používa sa ako miera diverzifikácie hodnôt vo výstupnom atribúte. Tak ako aj ostatné miery neusporiadanosti nadobúda hodnoty od 0 po 1, kde 0 znamená dokonalú rovnosť tried výstupu a 1 úplnú nerovnosť. Kritérium používajúce Gini index, ako funkciu neusporiadanosti $\gamma$, potom definujeme ako
\begin{align}
...
\right) ^ 2. 
\end{equation}
Pre binárny prípad (počet tried je rovný dvom) je výpočet kritéria jednoduchší.
\begin{equation}
...
Medzi najčastejšie využívané odhady generalizačnej chyby patria tie empirické. Pri tomto type odhadov sa vstupná množina rozdelí na trénovaciu a testovaciu podmnožinu. Rozdelenie býva najčastejšie náhodné a v pomere 2:1. Väčšia podmnožina je trénovacia a menšia zase testovacia. 
Model, ako už vieme, vytvárame pomocou trénovacej podmnožiny. Testovaciu podmnožinu využívame na vyhodnotenie kvality stromu a na získanie testovacej chyby $Err$. Táto testovacia chyba býva lepším odhadom generalizačnej chyby, než je tá trénovacia. Nevýhodou tohoto prístupu je menšia veľkosť trénovacej podmnožiny. Pri veľmi malej vzorke vstupných dát sa trénovacia aj testovacia podmnožina vytvára pomocou výberu s opakovaním. Na Obrázku \ref{fig:testtrain} je zobrazený vývoj trénovacej a testovacej chyby. Prerušovaná čiara v minime testovacej chyby predstavuje najlepší strom, ktorý môžeme získať pre vstupné dáta (nepreučený model s čo najlepšími generalizačnými schopnosťami).
...
Kritérium presnosti, ktoré sme spomínali v predchádzajúcej časti je síce najdôležitejším kritériom na vyhodnotenie modelu, no v niektorých prípadoch je jeho použitie úplne nevhodné. Príkladom, kedy je použitie kritéria presnosti nevhodné, je v prípade nerovnomerného rozdelenia hodnôt výstupného atribútu. Kritérium presnosti by v prípade tejto nerovnomernosti preferovalo modely, ktoré by vždy predikovali majoritnú hodnotu výstupného atribútu. V tomto prípade je nutné použiť nejaké z alternatívnych metrík.  
...
Indukčné algoritmy majú za úlohu vytvoriť rozhodovací strom zo zadanej trénovacej množiny $D$. Hlavnou úlohou pri tvorbe stromov je minimalizovať generalizačnú chybu (\ref{kap1:2.6:2.6.1:Generalize}). Nie vždy sa ale jedná len o minimalizovanie. Algoritmus sa môže snažiť o vytvorenie stromu s čo najnižšou výškou alebo tiež s obmedzeným počtom vrcholov.

Tvorba optimálneho rozhodovacieho stromu je ťažkou úlohou. Viac o tom v knihe \cite[s.51]{kap1-DataMiningForTrees}. Existujú dva typy indukčných algoritmov: indukčné algoritmy ktoré vytvárajú stromy zhora nadol a indukčné algoritmy, ktoré vytvárajú stromy zdola nahor. Medzi algoritmy typu zhora nadol patria ID3, C4.5, CART a mnoho ďalších. Tvorba stromu je pri niektorých algoritmoch (C4.5 a CART) spojená aj s jeho orezaním. Väčšinou na začiatku existuje jeden vrchol stromu (koreň), ktorému odpovedá celá trénovacia množina $D$. Následníci tohoto koreňa ďalej zodpovedajú podmnožinám trénovacej množiny. Týmto postupom stále delíme trénovaciu množinu, a tým vytvárame nové vrcholy stromu. Delenie trénovacej množiny vykonávame až dokým nie je pravdivé niektoré zo stanovených kritérií zastavenia. Kostra takejto tvorby stromu je popísaná v Algoritme  \ref{fig:genericAlgoritm}.
...
Techniky orezávania vykonávajú prechod stromu zhora nadol (od koreňa k listom) alebo naopak (od listov ku koreňu). Keď strom po orezaní vylepšil predom dané kritérium kvality stromu $\epsilon$, tak je orezaný strom prijatý.
...
\end{equation}
pričom $\epsilon(T_{i},D)$ je hodnota daného kritéria pre strom $T_{i}$ a dáta $D$. Hodnota $listy(T_{i})$ je množina listov stromu $T_{i}$ a $orezanie(T_{i},t)$ je funkcia, ktorá zmení vrchol $t$ stromu $T_{i}$ na list.
...
Jedným z ďalších vylepšení C4.5 je ešte o niečo novší, komerčný algoritmus C5.0. Popri výhodám z C4.5 obsahuje ešte ďalšie zlepšenia, ako je zrýchlenie výpočtu a zlepšenie práce s pamäťou. Ďalším zlepšením je vstavané použitie techniky boosting, ktorá dokáže zvýšiť prediktívne schopnosti modelu.
...
Algoritmus je schopný vytvárať regresné stromy, ktoré obsahujú v listoch reálne hodnoty. V tomto prípade sú deliace body vyberané tak, aby minimalizovali kvadratickú odchýlku predikovaných hodnôt od správnych hodnôt z dátovej množiny. V listových vrcholoch je regresia rovná váženému priemeru výstupnej hodnoty dát v tomto vrchole.

\section{Využitie rozhodovacích stromov pri predikcii}\label{kap1:2.8:DTUsage}
V tomto oddieli zhrnieme výhody a nevýhody rozhodovacích stromov, ktoré priamo ovplyvňujú použiteľnosť tohoto modelu v praxi. Zároveň predstavíme projekty, ktoré používajú techniku rozhodovacích stromov v praxi. Knihy s informáciami, ktoré boli použité v tejto časti, sú \cite{kap1-DataMiningAndAnalysis,kap1-DataMiningForTrees}. Informácie s praktickým použitím rozhodovacích stromov sme čerpali zo stránok príslušných projektov \cite{online-astronomy} a \cite{online-psychoterapy}.
...
\item Problém fragmentácie pri rozdeľovaní dát vo vrcholoch. K tomuto problému dochádza v prípade, keď dátové množiny vo vrcholoch rozdelíme na rovnako veľké podmnožiny. Strom, ktorý by takto rozdeľoval dátovú množinu veľkosti $n$, obsahuje na ceste z koreňa do listu iba $\log n$ vrcholov, čo je ekvivalentné s otestovaním maximálne $\log n$ atribútov. Toto býva veľký problém pri existencii väčšieho počtu významných atribútov.
\end{itemize}
...
Ako druhý príklad praktického nasadenia rozhodovacích stromov uvádzame ich použitie v medicíne. Cieľom projekte bolo uľahčiť prácu lekárom a zdravotnému personálu vytvorením modelu, ktorý by varoval pred neúspechom liečby a pomocou správnych rozhodnutí odporúčal, ako čo najlepšie predchádzať takémuto neúspechu. 
