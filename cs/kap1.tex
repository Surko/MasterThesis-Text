...
Generalizačná chyba $GE$ pre určitý model je definovaná ako pravdepodobnosť, že model nesprávne klasifikuje novú inštanciu (meria ako dobre model generalizuje nové dáta). Klasifikačná presnosť modelu $ACC$, ďalej len ako presnosť modelu je potom definovaná:
\begin{equation}
ACC = 1 - GE.
\end{equation}
...
Nadchádzajúce definície uvažujú, pre ľahšie pochopenie binárny prípad výstupného atribútu (pre n-árny prípad \ref{TODO}).

\begin{def-sk}[Confusion Matrix]
Nech $M$ je prediktívny model a $C$ matica, kde riadky predstavujú predikované triedy modelom $M$ a stĺpce skutočné hodnoty výstupného atribútu. Matica $C$ je potom špeciálna tabuľka používaná na posúdenie kvality modelu $M$ (angl. confusion matrix).
\end{def-sk}

\begin{itemize}
\item Citlivosť je schopnosť modelu rozpoznávať pozitívne inštancie
\item Špecificita je schopnosť modelu rozpoznávať negatívne inštancie
\item Precíznosť je schopnosť modelu rozpoznávať koľko pozitívne klasifikovaných inštancií je skutočne pozitívnych.
\item F-miera je schopnosť modelu rozpoznávať koľko pozitívne klasifikovaných inštancií je skutočne pozitívnych.
\end{itemize}
